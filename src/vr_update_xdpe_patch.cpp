...
vr_update_xdpe_patch::vr_update_xdpe_patch(
...
{
    std::array<uint8_t, INDEX_12> ProductId{0x90, 0x8A, 0x8C, 0x95, 0x96, 0x97,
                                            0x98, 0x99, 0x9C, 0x9A, 0x9E, 0x9B};
    ...
    CfgFileFd.read(reinterpret_cast<char*>(&PatchFileSize),
                   sizeof(PatchFileSize));
    ...
    CfgFileFd.read(reinterpret_cast<char*>(&FirmwareCode),
                   sizeof(FirmwareCode));
    ...
    rc = i2c_smbus_write_block_data(fd, BLOCK_PREFIX, (uint8_t)LENGTHOFBLOCK,
                                    wdata);
    if (rc != SUCCESS)
    {
    ...
    {
        sd_journal_print(
            LOG_ERR, "Not enough space to update the patch to the partition\n");
        return false;
    }
    return true;
}

bool WriteDataToScratchpad(std::string ConfigFilePath, uint16_t PatchFileSize,
                           int fd, bool EarlierControlRev)
{
    ...
    if (EarlierControlRev == false)
    {
        ...
        sd_journal_print(
            LOG_DEBUG,
            "Block write : Command = 0x%x Data = 0x%x 0x%x 0x%x 0x%x\n",
            BLOCK_PREFIX, wdata[INDEX_0], wdata[INDEX_1], wdata[INDEX_2],
            wdata[INDEX_3]);

        rc = i2c_smbus_write_block_data(fd, BLOCK_PREFIX,
                                        (uint8_t)LENGTHOFBLOCK, wdata);
    ...
    for (uint32_t i = 0; i < (PatchFileSize / INT_1024); i++)
    {
        sd_journal_print(
            LOG_INFO,
            "Block write: Command code = 0x%x Scratchpad data = 0x%x 0x%x 0x%x 0x%x\n",
            RPTR, ScratchpadAddr[INDEX_0], ScratchpadAddr[INDEX_1],
            ScratchpadAddr[INDEX_2], ScratchpadAddr[INDEX_3]);

        rc = i2c_smbus_write_block_data(fd, RPTR, (uint8_t)LENGTHOFBLOCK,
                                        ScratchpadAddr);
        ...
        for (uint32_t j = 0; j <= INT_1020; j += INDEX_4)
        {
            ...
            sd_journal_print(
                LOG_INFO,
                "Block write: Command code = 0x%x 0x%x 0x%x 0x%x 0x%x\n",
                MFR_REG_WRITE, wdata[INDEX_0], wdata[INDEX_1], wdata[INDEX_2],
                wdata[INDEX_3]);

            rc = i2c_smbus_write_block_data(fd, MFR_REG_WRITE,
                                            (uint8_t)LENGTHOFBLOCK, wdata);
            if (rc != SUCCESS)
            {
                sd_journal_print(LOG_ERR, "Error: Failed to write data\n");
...
bool vr_update_xdpe_patch::UpdateFirmware()
{
    ...
    std::array<uint8_t, INDEX_8> ProductId{0x90, 0x8A, 0x8C, 0x95,
                                           0x96, 0x97, 0x98, 0x99};
    ...
    if (length > LENGTH_0)
    {
        DeviceFwId = (rdata[INDEX_3] << SHIFT_24) |
    ...
    if (DeviceFwId == FW_ROM_ID)
    {
        sd_journal_print(
            LOG_INFO,
            "Device FW ID is 0x6192EE1E.Hence, No existing patch in the system\n");
    }
    else
    {
        sd_journal_print(
            LOG_INFO,
            "Patch already running in partition 1. Invalidating patch\n");

        if (EarlierCtrlRev == true)
        {
            sd_journal_print(LOG_INFO,
                             "Invalidating earlier controller revision\n");
            ...
            sd_journal_print(
                LOG_INFO,
                "Block write : Command = 0x%x Data = 0x%x 0x%x 0x%x 0x%x\n",
                RPTR, wdata[INDEX_0], wdata[INDEX_1], wdata[INDEX_2],
                wdata[INDEX_3]);
            rc = i2c_smbus_write_block_data(fd, RPTR, (uint8_t)LENGTHOFBLOCK,
                                            wdata);
            if (rc != SUCCESS)
            ...
            sd_journal_print(LOG_DEBUG,
                             "Write byte : Command = 0x%x Data = 0x%x\n",
        ...
        {
            sd_journal_print(LOG_INFO,
                             "Invalidating OTP for latest controller\n");
            memset(wdata, INDEX_0, MAXBUFFERSIZE);
        ...
        if (length > SUCCESS)
        {
            ...
            sd_journal_print(
                LOG_INFO, "Device firmware ID after invalidating OTP = 0x%x\n",
                DeviceFwId);
        }
        else
        {
    ...
    if (WriteDataToScratchpad(ConfigFilePath, PatchFileSize, fd,
                              EarlierCtrlRev) == false)
    {
        return false;
    }
