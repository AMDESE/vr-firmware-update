...
vr_update_renesas_gen2::vr_update_renesas_gen2(
...
bool vr_update_renesas_gen2::isUpdatable()
{
    ...
    if (ret >= SUCCESS)
    {
        DeviceRevision = (rdata[INDEX_4] << SHIFT_24) |
    ...
    {
        sd_journal_print(LOG_ERR,
                         "Failed to read device revision from the VR device\n");
        rc = false;
        goto Clean;
    }
    ...
    {
        sd_journal_print(LOG_ERR,
                         "Failed to read device revision from config file\n");
        rc = false;
        goto Clean;
    }
    ...
    if (DeviceRevision < DEVICE_REVISION)
    {
        sd_journal_print(
            LOG_ERR,
            "The device revision number is lesser than 2.0.0.3.Contact Renesas for support\n");
        rc = false;
        goto Clean;
    }

    if (((FileRevision >> SHIFT_24) & INT_255) ==
        ((DeviceRevision >> SHIFT_24) & INT_255))
    {
        sd_journal_print(LOG_INFO,
                         "File Revision matched with Device Revision\n");
    }
    else
    {
        sd_journal_print(
            LOG_ERR,
            "File Revision didnot match with Device Revision. Aborting the update\n");
        rc = false;
        goto Clean;
    }
    rc = true;
...
bool vr_update_renesas_gen2::ValidateFirmware()
{
    ...
    while (timeout < INDEX_10)
    {
        ...
        if (i2c_smbus_write_word_data(fd, DMA_WRITE, GEN2_PRGM_STATUS) ==
            SUCCESS)
        {
    ...
    if (status == FAILURE)
    {
        ...
        {
            sd_journal_print(LOG_ERR,
                             "the CRC check fails on the OTP memory\n");
        }
    ...
    for (ByteCount = INDEX_0; ByteCount < INDEX_4; ByteCount++)
    {
        ...
        for (int BankStatus = INDEX_0; BankStatus < INDEX_2; BankStatus++)
        {
            ...
            {
                sd_journal_print(LOG_ERR,
                                 "Bank %d : Bank Written (No Failures) \n",
                                 BankNumber);
            }
