...
int vrUpdate(std::string Model, uint16_t SlaveAddress, uint32_t Crc,
             std::string Processor, std::string MachineName,std::string configFilePath, std::string UpdateType)
{
    ...
    vr_update_obj = vr_update::CreateVRFrameworkObject(Model,
                    SlaveAddress,Crc,Processor,MachineName,configFilePath,UpdateType);
    ...
    else
    {
        ...
        if (std::filesystem::exists(vrBundleJsonFile))
        {
            ...
            for (json record : data["VR"])
            {
                ...
                ret = vrUpdate(Model,SlaveAddress,Crc,Processor,MachineName,configFilePath,UpdateType);
