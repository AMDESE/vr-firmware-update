...
vr_update_mp2869::vr_update_mp2869(
...
bool vr_update_mp2869::isUpdatable()
{
    ...
    if (ret >= SUCCESS)
    {
        VrVendorId = (rdata[INDEX_3] << SHIFT_16) |
    ...
    else
    {
        sd_journal_print(LOG_ERR,
                         "Vendor ID 0x%x mismatched. Aborting the update\n",
                         VrVendorId);
        return false;
    }
    ...
    {
        sd_journal_print(LOG_ERR,
                         "Device ID = 0x%x mismatched. Aborting the update\n",
                         VrDeviceId);
        return false;
    }
    ...
    {
        sd_journal_print(LOG_ERR,
                         "Error: Config ID mismatch. Update failed 0x%x 0x%x\n",
...
bool vr_update_mp2869::UpdateFirmware()
{
    ...
    {
        sd_journal_print(LOG_ERR,
                         "Error: Writing password to address 0xF7 failed\n");
        return false;
    }
    else
    {
    ...
    while (getline(cFile, line))
    {
        ...
        if (Mode == BLOCK_1)
        {
            ...
            else
            {
                sd_journal_print(
                    LOG_DEBUG,
                    "Byte write : Page = 0x%x Register = 0x%x , data = 0x%x\n",
    ...
    else
    {
        sd_journal_print(LOG_INFO,
                         "Successfully sent no byte command to page0@17h \n");
    }
...
bool vr_update_mp2869::ValidateFirmware()
{
    ...
    {
        sd_journal_print(LOG_INFO,
                         "CRC at page0@EDh matched with the Config file CRC\n");
    }
    else
    {
        sd_journal_print(
            LOG_ERR,
            "CRC at page0@EDh did not match with the Config file CRC. Aborting the update\n");
        return false;
    }
    ...
    {
        sd_journal_print(
            LOG_ERR, "Error: Sending no byte command to page0@18h failed\n");
        return false;
    }
    else
    {
        sd_journal_print(LOG_INFO,
                         "Successfully sent no byte command to page0@18h \n");
    }
