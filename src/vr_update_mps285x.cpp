...
vr_update_mps285x::vr_update_mps285x(
...
bool vr_update_mps285x::isUpdatable()
{
    ...
    {
        sd_journal_print(
            LOG_ERR, "Error: Failed to read Vendor ID from the VR device\n");
        return false;
    }
    ...
    {
        sd_journal_print(
            LOG_ERR, "Error: Failed to read Device ID from the VR device\n");
        return false;
    }
    ...
    {
        sd_journal_print(
            LOG_ERR, "Error: Failed to read Config ID from the VR device\n");
        return false;
    }
    ...
    if (cFile.is_open())
    {
        while (getline(cFile, line))
        {
            if (line.find(FILE_VENDOR_ID) != std::string::npos)
            {
                ...
                std::string Id =
                    line.substr(pos + sizeof(FILE_VENDOR_ID), INDEX_4);
                ss << Id;
            ...
            else if (line.find(FILE_DEVICE_ID) != std::string::npos)
            {
                ...
                std::string Id =
                    line.substr(pos + sizeof(FILE_DEVICE_ID), INDEX_4);
                ss << Id;
            ...
            else if (line.find(FILE_CONFIG_ID) != std::string::npos)
            {
                ...
                std::string Id =
                    line.substr(pos + sizeof(FILE_CONFIG_ID), INDEX_4);
                ss << Id;
    ...
    if ((VrVendorId == FileVendorId) && (VrDeviceId == FileDeviceId) &&
    ...
    else
    {
        sd_journal_print(
            LOG_ERR,
            "Error: VR IDs and File devie IDs did not match. Update failed\n");
        cFile.close();
        return false;
    }
...
bool vr_update_mps285x::UpdateFirmware()
{
    ...
    {
        sd_journal_print(
            LOG_ERR, "Error: Failed to read password unlock register data\n");
        return false;
    }

    sd_journal_print(LOG_INFO, "Password unlock register - Page0 0x%x = 0x%x",
                     PWD_UNLOCK_REG, word_data);

    if (word_data & (INDEX_1 << INDEX_3) == 0)
    {
        sd_journal_print(
            LOG_INFO,
            "The device is password locked. Reading pwd from Page 0 7Eh\n");
        ...
        if (word_data < SUCCESS)
        {
            sd_journal_print(LOG_ERR,
                             "Error: Failed to read password register\n");
            return false;
        }

        sd_journal_print(LOG_INFO,
                         "Writing password 0x%x to unlock the device \n",
                         word_data);
        ...
        {
            sd_journal_print(
                LOG_ERR, "Error: Writing password to address 0xF8 failed\n");
            return false;
        }
        ...
        if (ByteData < SUCCESS)
        {
            sd_journal_print(
                LOG_ERR,
                "Error: Failed to read password unlock register data\n");
            return false;
        }

        sd_journal_print(
            LOG_INFO,
            "After writing password : Password unlock register Page0 7Eh = 0x%x",
            ByteData);
        ...
        else
        {
            sd_journal_print(
                LOG_INFO,
                "Password unlocking failed for the device. Aborting the update\n");
            return false;
        }
    }
    ...
    if (ret >= SUCCESS)
    {
        ...
        if ((rdata[INDEX_0] & INDEX_4) == INDEX_1)
        {
            ...
            {
                sd_journal_print(LOG_ERR,
                                 "Error: Unlocking memory protection failed\n");
                return false;
            }
        }
        else
    ...
    while (getline(cFile, line))
    {
        ...
        for (int i = 0; i < TOTAL_COLUMNS; i++)
        {
            ...
            if (i == INDEX_1)
            {
                ...
                if (current_page != Page)
                {
                    if (Page == INDEX_2)
                    {
                        sd_journal_print(
                            LOG_INFO,
                            "Sending F1h command to store data to MTP\n");
                        ...
                        if (ret < SUCCESS)
                        {
                            sd_journal_print(
                                LOG_ERR,
                                "Error: Sending F1h command to store data to MTP failed\n");
                            cFile.close();
                            return false;
                        }
                        ...
                        sleep(1);
                        uint16_t byte_data =
                            i2c_smbus_read_word_data(fd, CMD_4F);

                        sd_journal_print(LOG_INFO,
                                         "4Fh data read from the device 0x%x\n",
                                         byte_data);

                        if (byte_data < SUCCESS)
                        {
                            sd_journal_print(
                                LOG_ERR, "Error: Failed to read 0x4F data\n");
                            return false;
                        }
                        ...
                        if (ret < SUCCESS)
                        {
                            sd_journal_print(
                                LOG_ERR,
                                "Error: Writing back to command 0x4F failed\n");
                            cFile.close();
                            return false;
                        }
                    }
                    ...
                    sd_journal_print(LOG_INFO,
                                     "Setting current page number to %x\n",
                                     current_page);

                    ret = i2c_smbus_write_byte_data(fd, SET_PAGE_REG,
                                                    current_page);
                    ...
                    if (ret < SUCCESS)
                    {
                        sd_journal_print(
                            LOG_ERR,
                            "Error: Writing byte data to the device failed\n");
                        cFile.close();
                        return false;
                    }
                    ...
                    if (Page == INDEX_2)
                    {
                        ...
                        sd_journal_print(
                            LOG_INFO,
                            "Written all 0 and 1 register. Sent F3h command\n");

                        if (ret < SUCCESS)
                        {
                            sd_journal_print(
                                LOG_ERR, "Error: Sending F3h command failed\n");
                            cFile.close();
                            return false;
                        }

                        sd_journal_print(LOG_INFO,
                                         "Setting page number to 0x2A\n");
                        ret = i2c_smbus_write_byte_data(fd, SET_PAGE_REG,
                                                        PAGE_2A);
            ...
            else if (i == INDEX_2)
            {
                ...
                ss >> Register;
                ss.clear();
                if ((std::find(RegisterSet.begin(), RegisterSet.end(),
                               Register) != RegisterSet.end()) &&
                    (current_page == 2))
                {
                    Data = 0;
                    break;
        ...
        if (Mode == BLOCK_1)
        {
            ...
            {
                sd_journal_print(
                    LOG_ERR, "Error: Writing byte data to the device failed\n");
                cFile.close();
                return false;
            }
        }
        ...
        else if (Mode == BLOCK_2)
        {
            ...
            {
                sd_journal_print(
                    LOG_ERR, "Error: Writing word data to the device failed\n");
                cFile.close();
                return false;
            }
        }
        ...
        else if (Mode == BLOCK_3)
        {
            ...
            if (ret < SUCCESS)
            {
                sd_journal_print(
                    LOG_ERR,
                    "Error: Writing block data to the device failed   %x   0x%x 0x%x\n",
        ...
        {
            sd_journal_print(LOG_ERR,
                             "Error: Invalid Mode: %d for Register: %x \n",
                             Mode, Register);
            cFile.close();
            return false;
        }
...
bool vr_update_mps285x::ValidateFirmware()
{
    ...
    sd_journal_print(LOG_INFO,
                     "Config file : User CRC = 0x%x Multi CRC = 0x%x\n",
                     UserCrc, MultiCrc);
