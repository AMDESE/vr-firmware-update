...
vr_update* vr_update::CreateVRFrameworkObject(std::string Model,
              ...
              std::string MachineName, std::string configFilePath,std::string UpdateType)
{
	vr_update* p;

    if ((strcasecmp(UpdateType.c_str(), PATCH)) == SUCCESS)
    {
        if ((Model.compare(RAA229613) == SUCCESS) ||
             (Model.compare(RAA229625) == SUCCESS)||
             (Model.compare(RAA229620) == SUCCESS)||
             (Model.compare(ISL68220) == SUCCESS) ||
             (Model.compare(RENESAS) == SUCCESS))
        {
