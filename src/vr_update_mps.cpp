...
bool vr_update_mps::crcCheckSum()
{
    ...
    if (DeviceCrc == Crc)
    {
        sd_journal_print(
            LOG_ERR,
            "Error: Device CRC matches with file CRC. Skipping the update\n");
        CrcMatched = true;
        return false;
    }
    else
    {
        sd_journal_print(
            LOG_ERR,
            "Info: CRC does not match with the previous image. Continuing the update\n");
        CrcMatched = false;
        return true;
    }
}
...
bool vr_update_mps::isUpdatable()
{
    ...
    if (ret >= SUCCESS)
    {
        VrVendorId = (rdata[INDEX_0] << SHIFT_24) |
                     (rdata[INDEX_1] << SHIFT_16) |
    ...
    if (ret >= SUCCESS)
    {
        VrConfigId = (rdata[INDEX_0] << SHIFT_16) |
                     (rdata[INDEX_1] << SHIFT_8) | rdata[INDEX_2];
    }
    else
    {
        sd_journal_print(
    ...
    if (cFile.is_open())
    {
        while (getline(cFile, line))
        {
            if (line.find(FILE_VENDOR_ID) != std::string::npos)
            {
                std::stringstream ss;
                std::string Id =
                    line.substr(sizeof(FILE_VENDOR_ID) + INDEX_2, 8);
                ss << Id;
            ...
            else if (line.find(FILE_DEVICE_ID) != std::string::npos)
            {
                std::stringstream ss;
                std::string Id =
                    line.substr(sizeof(FILE_DEVICE_ID) + INDEX_2, 4);
                ss << Id;
            ...
            else if (line.find(FILE_CONFIG_ID) != std::string::npos)
            {
                std::stringstream ss;
                std::string Id =
                    line.substr(sizeof(FILE_CONFIG_ID) + INDEX_2, 6);
                ss << Id;
    ...
    while (getline(cFile, line))
    {
        ...
        if (Mode == BLOCK_1)
        {
            ...
            if (ret < SUCCESS)
            {
                sd_journal_print(
                    LOG_ERR,
                    "Error: Writing block data to the device failed\n");
                cFile.close();
                return FAILURE;
            }
        }
        ...
        else if (Mode == BLOCK_3)
        {
            ...
            ret = i2c_smbus_write_i2c_block_data(fd, Register, BYTE_COUNT_3,
                                                 rxdata);
            if (ret < SUCCESS)
            {
                sd_journal_print(
