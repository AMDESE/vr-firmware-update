...
vr_update_renesas_patch::vr_update_renesas_patch(
    ...
    if (ret == SUCCESS)
    {
        ...
        if (ret >= SUCCESS)
        {
            ...
            if (DeviceId == CANDIDATE_FW)
            {
                sd_journal_print(
                    LOG_INFO,
                    "Patch update is allowed for this device. Updating the patch\n");
                return true;
            }
            else
            {
                sd_journal_print(
                    LOG_ERR,
                    "Patch update is not allowed for this device. Aborting the update\n");
                return false;
            }
        }
        else
    ...
    if (newfile.is_open())
    {
        ...
        while (getline(newfile, HexfileData))
        {
            ...
            else
            {
                ...
                {
                    sd_journal_print(
                        LOG_ERR, "Writing word data to the device failed\n");
                    return false;
                }
            }
        }
            ...
            if (ret >= SUCCESS)
            {
                if ((rdata[INDEX_3] & COMPLETE_BIT))
                {
                    if ((rdata[INDEX_3] & PASS_BIT))
                    {
                        ...
                        sd_journal_print(
                            LOG_ERR,
                            "Patch update Succeeded. Please do AC cycle\n");
                        break;
                    }
