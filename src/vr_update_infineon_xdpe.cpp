...
vr_update_infineon_xdpe::vr_update_infineon_xdpe(
...
bool vr_update_infineon_xdpe::crcCheckSum()
{
    ...
    {
        DeviceCrcData =
            ((uint32_t)rdata[INDEX_3] << SHIFT_24) |
...
bool vr_update_infineon_xdpe::isUpdatable()
{
    ...
    {
        sd_journal_print(LOG_ERR,
                         "Available space is less, program manually\n");
        return false;
    }
    else
    {
...
std::vector<std::vector<std::string>> get_each_section_data(std::string data)
{
    ...
    data = data.substr(start_delim + start_pat.length(),
                       end_delim - start_delim - start_pat.length());
    ...
    {
        unsigned second = data.find(
            "//XV",
            first + INDEX_2); // Adjust position of index to get required data
        std::string blob =
            data.substr(first + INDEX_2, second - first - INDEX_2);
        ...
        while (getline(ss, line))
        {
            if (size_t foundxv = line.find("XV") != std::string::npos)
            {
                ...
                if (size_t foundxv = line.find("XV0 Partial") !=
                                     std::string::npos)
                {
            ...
            {
                std::string row_num = line.substr(
                    INDEX_0,
                    INDEX_4); // Row number always contains 3 digits hence (0,4)
                if (size_t foundrn = line.find(row_num) != std::string::npos)
                {
                    line.erase(foundrn - INDEX_1, row_num.length());
                    n_blob.append(line + " ");
                    if (partial_section_found == true &&
                        (row_num.compare("000 ") == SUCCESS))
                    {
                        std::string crc = line.substr(13, INDEX_4);
                        partial_crc = partial_crc +
                                      std::stoi(crc, NULL, BASE_16);
                    }
                }
            }
        }
    ...
    sd_journal_print(LOG_DEBUG,
                     "Partial section number = %d Partial CRC = %d \n",
...
int invalidateOtp(uint8_t xv, uint8_t hc, int fd)
{
    ...
    sd_journal_print(LOG_DEBUG, "data: 0x%x 0x%x 0x%x 0x%x\n", wdata[INDEX_3],
                     wdata[INDEX_2], wdata[INDEX_1], wdata[INDEX_0]);
...
int writeDataToScratchpad(std::vector<std::string> section, int fd)
{
    ...
    sd_journal_print(LOG_DEBUG,
                     "ScratchPad Initial Block Write with cmd 0xce\n");
    sd_journal_print(LOG_DEBUG, "data: 0x%x 0x%x 0x%x 0x%x\n", wdata[INDEX_3],
                     wdata[INDEX_2], wdata[INDEX_1], wdata[INDEX_0]);
    ...
    sd_journal_print(LOG_DEBUG, "SCPAD Address 0x%x 0x%x 0x%x 0x%x\n",
                     wdata[INDEX_3], wdata[INDEX_2], wdata[INDEX_1],
                     wdata[INDEX_0]);
    ...
    {
        sd_journal_print(
            LOG_ERR, "Error: Failed to write SCPAD Address to RPTR, rc=%d\n",
            rc);
        return FAILURE;
    }

    sd_journal_print(LOG_DEBUG,
                     "Write Block Data to Scratchpad with cmd 0xde\n");
    ...
    for (int i = 0; i < section.size(); i++)
    {
        ...
        sd_journal_print(LOG_DEBUG, "0x%x 0x%x 0x%x 0x%x\n", sdata[INDEX_0],
                         sdata[INDEX_1], sdata[INDEX_2], sdata[INDEX_3]);

        rc = i2c_smbus_write_block_data(fd, MFR_REG_WRITE,
                                        (uint8_t)LENGTHOFBLOCK, sdata);
        usleep(MINWAITTIME);
        if (rc != SUCCESS)
        {
            sd_journal_print(
